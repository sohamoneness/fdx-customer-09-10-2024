...
public class PreOrderOnlyAdapter extends RecyclerView.Adapter<PreOrderOnlyAdapter.Hold> {
    ...
    public PreOrderOnlyAdapter(DashboardHome dashboardHome, List<RestaurantModel> nearbyRestList) {
    ...
    public void onBindViewHolder(@NonNull Hold holder, int position) {
        ...
        int hrs = Integer.parseInt(nbrM.pre_order_duration)/60;
        if (hrs < 24){
            holder.tvTime.setText(hrs + " hrs");
        }else {
            if (hrs % 24 == 0){
                int day = hrs / 24;
                ...
                int day = hrs / 24;
                int left_hrs = hrs - (day*24);
                holder.tvTime.setText(day + " days " + left_hrs + " hrs");
